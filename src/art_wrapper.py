# This is a wrapper by Mladen Karan <m.karan@qmul.ac.uk> around the below code
...
# src_words and trg_words are the words and x,z are source and target embeddings
# the return value is a tuple xw, zw which are the two aligned embedding matrices

def run_supervised_alignment(src_words, trg_words, x, z, src_indices, trg_indices, supervision = "--supervised"):
...
def main():
    dictionary_filename = "./data/dictionaries/en-es.train.txt"
    src_input_filename = "./data/embeddings/en.emb.txt"
    tar_input_filename = "./data/embeddings/es.emb.txt"

    # load up the embeddings
    dtype = "float32"
    srcfile = open(src_input_filename, encoding="utf-8", errors='surrogateescape')
    trgfile = open(tar_input_filename, encoding="utf-8", errors='surrogateescape')
    src_words, x = embeddings.read(srcfile, dtype=dtype)
    trg_words, z = embeddings.read(trgfile, dtype=dtype)
    ...
    f = open(dictionary_filename, encoding="utf-8", errors='surrogateescape')
    for line in f:
        src, trg = line.split()
        try:
            src_ind = src_word2ind[src]
    ...
    xw, zw = run_supervised_alignment(src_words, trg_words, x, z, src_indices, trg_indices)    
    print("All done ^^. Time: "  + str(time.time() - start_time))
 
    # write res to disk
    src_output_filename = "./SRC_OUTPUT_FROMCODE.txt"
    tar_output_filename = "./TAR_OUTPUT_FROMCODE.txt"
